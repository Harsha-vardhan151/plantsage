...
export default function ResultsView({ data, previewUrl }: { data: AnalyzeResp | null; previewUrl: string | null }) {
  if (!data) return null;
  const low = data.species.length > 0 && data.species[0].confidence < 0.35;
  ...
  return (
    <>
      ...
      {low && <div className="card warn">🤔 Low confidence. Try another photo, close-up, good light.</div>}
    </>
  );
}
